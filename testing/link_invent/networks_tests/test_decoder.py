...
from reinvent_models.link_invent.dataset.paired_dataset import PairedDataset, PairedModelVocabulary
...
from testing.fixtures.test_data import ETHANE, METAMIZOLE, PROPANE, IBUPROFEN, BUTANE, ASPIRIN, WARHEAD_PAIR, CELECOXIB
...
class TestDecoder(unittest.TestCase):
    def setUp(self) -> None:
        ...
        paired_model_vocabulary = PairedModelVocabulary.from_lists(self.smiles_list_input, self.smiles_list_target)
        self.voc_size_input, self.voc_size_target = paired_model_vocabulary.len()
        dataset = PairedDataset([[i, t] for i, t in zip(self.smiles_list_input, self.smiles_list_target)],
                                vocabulary=paired_model_vocabulary)

        self.data_loader_a = tud.DataLoader(dataset, batch_size=100, shuffle=False, collate_fn=dataset.collate_fn)
        self.data_loader_b = tud.DataLoader(dataset, batch_size=1, shuffle=False, collate_fn=dataset.collate_fn)
    ...
    def test_forward_normal_batch_no_singleton_dimensions(self):
        self._test_combination(self.data_loader_a, self.encoder_1, self.decoder_1,
                               self.parameter_1)
    ...
    def test_forward_normal_batch_size_1_with_singleton_dimensions(self):
        self._test_combination(self.data_loader_b, self.encoder_2, self.decoder_2,
                               self.parameter_2)

    def _test_combination(self, data_loader: tud.DataLoader, encoder: Encoder, decoder: Decoder, parameter: dict):

        for (seqs_input, seq_lengths_input), (seqs_target, seq_lengths_target) in data_loader:
            seqs_padded_enc, hidden_states = encoder.forward(seqs_input, seq_lengths_input)
            logits, hidden_states, weights = decoder.forward(seqs_target, seq_lengths_target, seqs_padded_enc,
                                                             hidden_states)
            batch_size = min(data_loader.batch_size, len(self.smiles_list_target))
            expected_size_logits = [batch_size, seq_lengths_target.max().item(), self.voc_size_target]
            expected_size_hidden_state = [parameter['num_layers'], batch_size, parameter['num_dimensions']]
            expected_size_weights = [batch_size, seq_lengths_target.max().item(), seq_lengths_input.max().item()]
