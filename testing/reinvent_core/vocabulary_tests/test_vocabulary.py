...
import reinvent_models.reinvent_core.models.vocabulary as mv
...
class TestVocabulary(unittest.TestCase):
    ...
    def setUp(self):
        self.voc = mv.Vocabulary(tokens=SIMPLE_TOKENS)
    ...
    def test_equal(self):
        self.assertEqual(self.voc, mv.Vocabulary(tokens=SIMPLE_TOKENS))
        self.voc.add("#")
        self.assertNotEqual(self.voc, mv.Vocabulary(tokens=SIMPLE_TOKENS))
    ...
    def test_decode(self):
        self.assertEqual(
            self.voc.decode(np.array([0, 8, 9, 8, 1])),
            ["$", "C", "F", "C", '^']
        )
